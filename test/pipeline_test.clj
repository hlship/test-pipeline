...
(defn ^:private expect-counters-to-change
  [context]
  (let [counters (test-reporting/snapshot-counters)]
    (p/continue context)
    (if (= counters
           (test-reporting/snapshot-counters))
      (clojure.test/do-report {:type     :fail
                               :message  "Expected test failures but there were none recorded"
                               :expected (list `not counters)
                               :actual   (test-reporting/snapshot-counters)})
      (roll-back-test-failure-counters counters))))

(deftest reporting-macro
  (p/execute
    expect-counters-to-change
    p/halt-on-failure
    (fn [context]
...
(deftest matches-macro
  (p/execute
    p/halt-on-failure
    expect-counters-to-change
    (fn [context]
      (p/continue (assoc context :data {:value 42})))
    ;; This will be reported as an error
    (p/matches? context
                {:value 99}
                (:data context))))

(deftest matches-macro-with-message
  (p/execute
    p/halt-on-failure
    expect-counters-to-change
    (fn [context]
